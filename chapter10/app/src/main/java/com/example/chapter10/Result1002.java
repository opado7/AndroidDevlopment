...
public class Result1002 extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {

        super.onCreate(savedInstanceState);
        setContentView(R.layout.result1002);
        setTitle("투표 결과");
        ...
        List<Integer> samePic = new ArrayList<>();
        for (int j = 1; j < voteResult.length; j++) {
            if (voteResult[maxEntry] == voteResult[j]) {
                samePic.add(j);
            }
        }
        ...
        if (samePic.size() > 1) {
            ...
            StringBuilder same_text = new StringBuilder("최대투표 ");
            same_text.append(String.valueOf(samePic.size()));
            same_text.append("명: [");
            for(int i = 0; i < samePic.size(); i++) {
                same_text.append(imageName[samePic.get(i)]);
                if (i != samePic.size() - 1) {
                    same_text.append(", ");
                }
            }
            same_text.append("]");
            tvSame.setText(same_text);
        }
